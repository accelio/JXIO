...
extern "C" JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM *jvm, void* reserved)
{
	...
	if (!xio_get_opt(NULL, XIO_OPTLEVEL_RDMA, XIO_OPTNAME_RDMA_NUM_DEVICES, &opt, &optlen) && (optlen <= 4) && (opt > 0)) {
		...
		opt = 512;
		if (xio_set_opt(NULL, XIO_OPTLEVEL_ACCELIO, XIO_OPTNAME_MAX_INLINE_DATA, &opt, sizeof(opt))) {
			bridge_print_error("failed to change Accelio's RDMA_BUF_THRESHOLD");
		}

		opt = 0;
		if (xio_set_opt(NULL, XIO_OPTLEVEL_ACCELIO, XIO_OPTNAME_MAX_INLINE_HEADER, &opt, sizeof(opt))) {
			bridge_print_error("failed to change Accelio's RDMA_BUF_THRESHOLD");
		}
	}
	else {
...
extern "C" JNIEXPORT jint JNICALL Java_com_mellanox_jxio_impl_Bridge_clientSendReqNative(JNIEnv *env, jclass cls, jlong ptr_session, jlong ptr_msg, jint out_size, jint in_size, jboolean is_mirror)
{
	Msg * msg = (Msg*) ptr_msg;
