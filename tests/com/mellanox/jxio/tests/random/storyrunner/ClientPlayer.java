...
public class ClientPlayer extends GeneralPlayer {
	...
	private class SendMsgTimer extends TimerList.Timer {
		...
		public void onTimeOut() {
			...
			while (numMsgToSend > 0) {
				...
				if (LOG.isDebugEnabled())
					LOG.debug(outer.toString() + ": sendRequest(" + msg + ")");
	...
	class JXIOCallbacks implements ClientSession.Callbacks {
		...
		public void onResponse(Msg msg) {
			...
			if (LOG.isDebugEnabled())
				LOG.debug(outer.toString() + ": onResponse(#" + outer.counterReceivedMsgs + ", " + msg + ")");
			msg.returnToParentPool();
		}
		...
		private long roundTrip(Msg m) {
			...
			if (LOG.isDebugEnabled()) {
				LOG.debug(outer.toString() + ": roundTrip for message " + outer.counterReceivedMsgs + " took " + rTrip
				        / 1000 + " usec");
			}
			return rTrip;
		}
